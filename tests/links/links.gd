...
func _test_links():
    ...
    Firebase.DynamicLinks.connect("dynamic_link_generated",Callable(self,"print_link"))
    ...
    await self.link_printed
    $unguessable_link_check.button_pressed = true
    ...
    await self.link_printed
    $guessable_link_check.button_pressed = true
