...
func begin_transaction():
	var firestore_task : FirestoreTask = FirestoreTask.new()
	firestore_task.connect("transaction_begun", self, "_on_transaction_begun")
	firestore_task.connect("task_error", self, "_on_task_error")
	firestore_task.action = FirestoreTask.Task.TASK_TRANSACTION
	var url : String = _base_url + _extended_url + ":beginTransaction"
